...
import org.apache.spark.sql.delta.{DeltaHistoryManager, DeltaLog, PreprocessTableOptimize}
...
trait DeltaTableOperations extends AnalysisHelper { self: DeltaTable =>
  ...
  protected def executeOptimize(
    ...
    val resolvedOptimize = PreprocessTableOptimize.resolveReferences(
      deltaOptimize, tryResolveReferences(sparkSession)(_, deltaOptimize))
